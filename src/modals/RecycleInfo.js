    ...
    return (
      <div key={i} className="recycle-item">
        ...
        <button
          ...
          style={{ backgroundColor: props.pref.color.highlight }}
          onClick={() => handleRetrieve(column)}
        >
          Retrieve List
        </button>
        <button
          className="delete-btn"
          style={{ backgroundColor: props.pref.color.warning }}
    ...
    <div className="card-modal-outer">
      <div className="card-modal-inner">
        <div className="recycle-head">
          ...
          <span className="spacer"></span>
          <h1 className="close-modal" onClick={props.close}>
            <span aria-hidden="true">&times;</span>
          </h1>
        </div>
        <div className="recycle-item-container">{display}</div>
        {!display.length ? (
          <p className="recycle-msg">Nothing in the recycle bin</p>
        ) : null}
      </div>
    </div>
  );
